{
	"translatorID": "RUSSIAN0-0BIB-TEX0-V002-TRANSLATOR00",
	"label": "BibTeX Russian",
	"creator": "Simon Kornblith and Richard Karnesky with tweaks by Tan and Ivan Denisov",
	"target": "bib",
	"minVersion": "2.1.9",
	"maxVersion": "",
	"priority": 200,
...
*/
var citeKeyFormat = "%Au%yr";
var citeKeyCollisionFormat = "%a";
var citeKeyField = "callNumber";  //e.g. callNumber
var exportLocalFilePath = true // default=no
var preserveLatex = true // default=no
// -- Tan --end

// Denisov Start
var allData = false // allows to get small BibTex for Natural Science citation in LaTeX
// Denisov End
...
function processField(item, field, value) {
	...
	} else if(field == "pages" || field == "number-of-pages") {
	...
	} else if(field == "keywords" && allData)  {
		if(value.indexOf(",") == -1) {
			// keywords/tags
...
// Ivan Denisov addon for Russian
	...
	"AU":function (label, maxlength, num, item) {
		if(item.creators && item.creators[num] && item.creators[num].lastName) {
			...
			if (maxlength && auth.length > maxlength) {
				auth = auth.substr(0,maxlength);
			}
			return translit(auth); // I.Denisov mod
		}
		return "";
	},
...
// -- Tan --end
		...
		// Denisov Start
		if(item.place) {
			if(item.place == "Москва" || item.place == "М")
				writeField("address", "М.");
			else if (item.place == "Санкт-Петербург" || item.place == "С.-П." || item.place == "СПб")
				writeField("address", "СПб.");
			else
				writeField("address", item.place);
		}
		// Denisov End
		...
		if(item.date) {
			...
			if(typeof date.month == "number" && allData) {
		...
		if(item.tags && item.tags.length && allData) {
			var tagString = "";
		...
		// In Russian mode we want a books number of pages in the BibTeX "pages" field for books.
		if(item.numPages) {
